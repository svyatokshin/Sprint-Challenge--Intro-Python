...
def cityreader(cities=[]):
    with open("src/cityreader/cities.csv", newline='') as csvfile:
        ...
        for row in data:
            city = City(f"{row['city']}", float(row['lat']), float(row['lng']))
            cities.append(city)
...
def cityreader_stretch(lat1, lon1, lat2, lon2, cities=[]):
    ...
    for city in cities:
        # compare inputs with cities latitude
        if city.lat <= max(lat1, lat2) and city.lat >= min(lat1, lat2):
            # compare inputs with cities longitude
            if city.lon <= max(lon1, lon2) and city.lon >= min(lon1, lon2):
                within.append(city)
                print(
                    f"{city.name} is within the range of latitudes and longitudes you chose")
            else:
